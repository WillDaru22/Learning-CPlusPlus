// Learning C++ Project 4 - WillDaru22
...
void locationMenu(const LocationList& locations) {
    int inp = 0;
    int num_loc = locations.GetCount();
    do {
        ...
        if(inp > 0 && inp < num_loc) {  // view details of selection
            do {
        ...
        else if(inp == num_loc+1) {  // back to menu
            return;
        }
...
void changeSettings(User& user) {
    ...
    do {
        ...
        else if (entry == 2) {  // save
            user.Write(metric);  // using this just to have something, metric isnt used in the function
            std::cout << "Saved data\n";
        }
        else if (entry == 3) {  // load
            std::cout << "Enter name to load data from\n";
            std::string lname;
            std::cin >> lname;
            if(std::cin.fail()) {  // failure check
                std::cin.clear();
...
int main(int argc, const char* argv[]) {
    ...
    bool dummy = true;
    current_user->Read(dummy, "default.txt");
    ...
    current_locations->FromFile("locations.txt");  // not working for some reason
    ...
    do {
        ...
        if(choice == 1) {  // locations menu
            ...
            current_user->Write(dummy);
            current_locations->ToFile("locations.txt");
            exit(0);
        }
